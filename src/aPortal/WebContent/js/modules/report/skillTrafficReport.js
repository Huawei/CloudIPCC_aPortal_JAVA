define(
		[ 'tac', 'tacCfg',
				'text!../../../html/modules/report/skillTrafficReport.html' ],
		function(tac, tacCfg, html) {
			 ...
			 */
			var eventList = [ {
				target : 'skillTrafficReport_searchBtn',// 查询
				attach : 'on',
				type : 'click',
				action : function(e) {
				...
				target : 'skillTrafficReport_resetBtn',// 重置
				attach : 'on',
				type : 'click',
				action : function(e) {
					$('#skillTrafficReport_startTime').val('');
					$('#skillTrafficReport_endTime').val('');
					$('#skillTrafficReport_type').val(2);
					_resetDisplayInfo();
				}
			},{
    			target : 'skillTrafficReport_exportBtn',//export
    			attach : 'on',
    			type : 'click',
    			action : function(e) {
    				_actionWaitFlag = true;
					...
					if (_checkParam())
					{
						...
						$("#skillTrafficReport_skills").val(selectedSkills);
						var myForm = document.getElementById('skillTrafficReport_form');
						myForm.submit();
					};					
					
    			}
    		}, {
				target : 'skillTrafficReport_startTime',// 开始时间
				attach : 'on',
				type : 'focus',
				action : function(e) {
					var lang = 'zh-cn';
					WdatePicker({
						...
						maxDate:'#F{$dp.$D(\'skillTrafficReport_endTime\',{d:-1})||\'%y-%M-{%d-1}\'}',  //开始时间最大日期为结束时间前一天
						lang : lang
					});
				}
			}, {
				target : 'skillTrafficReport_endTime',// 结束时间
				attach : 'on',
				type : 'focus',
				action : function(e) {
					var lang = 'zh-cn';
					WdatePicker({
						...
						readOnly : true,
						dateFmt : 'yyyy-MM-dd',
						minDate:'#F{$dp.$D(\'skillTrafficReport_startTime\',{d:1})||\'%y-%M-{%d-90}\'}',  //结束时间最小日期为开始时间后一天
			...
			function _resetDisplayInfo() {
				//重置技能队列
				$('#skillTrafficReport_queue').empty();
				$('#skillTrafficReport_queue').multiselect("destroy");
				_queryVoiceSkills();
				//重置报表信息
				$('#skillTrafficReport_content').empty();
				$('#skillTrafficReport_content').append(
					...
					success : function(__data) {
						if (__data.retcode == RestRetCode.SUCCESS) {
							...
							if (result && result.length > 0){								
								...
								$('#skillTrafficReport_queue').append(buffer.join(''));	
								
							}
						}
						//设置技能队列多选
						$("#skillTrafficReport_queue").multiselect({
			...
			var _isAllSkillsSelected = function() {
				var all = $("#skillTrafficReport_queue option").length;
			...
			var _getSkillsSelected = function() {
				if (_isAllSkillsSelected()) {
					...
					var selectedAgents = $('#skillTrafficReport_queue').val();
			...
			function _checkInputParams() {
				_startTime = $("#skillTrafficReport_startTime").val()+"";
				_endTime = $("#skillTrafficReport_endTime").val()+"";
				_rptType = $('#skillTrafficReport_type').val();
				_skills = $('#skillTrafficReport_queue').val();
			...
			var _queryReport = function() {
				var selectedSkills = _getSkillsSelected();		
				var requestParam = {
					'startTime' : $('#skillTrafficReport_startTime').val(),
					'endTime' : $('#skillTrafficReport_endTime').val(),
					'reportType' : $('#skillTrafficReport_type').val(),
				...
				tac.ajax({
							url : tacCfg.url.agentRestServer
									+ tacCfg.service.report.skillTraffic,
							type : 'POST',
							data : JSON.stringify(requestParam),
							contentType : 'application/json',
							loading : true,
								...
								if (__data.retcode == RestRetCode.SUCCESS) {
									...
									if (result && result.length > 0) {
										...
										for (var i = 0; i < result.length; i++) {
											...
											buffer.push('</td>');
											buffer.push('<td>');
											buffer.push(result[i]['maxQueueTime']);  //21.最大等待时长
											buffer.push('</td>');
											buffer.push('<td>');
											buffer.push(result[i]['minQueueTime']);  //22.最小等待时长
											buffer.push('</td>');
											buffer.push('</tr>');
											$('#skillTrafficReport_content').append(buffer.join(''));
										}
									} else {
										$('#skillTrafficReport_content').empty();
										$('#skillTrafficReport_content').append(
			...
			var _bindingEnterEvent = function() {
				$("#skillTrafficReport_type").keyup(function(event) {
					if (event.keyCode === 13)
						$("#skillTrafficReport_type").click();
				});

				$("#skillTrafficReport_startTime").keyup(function(event) {
					if (event.keyCode === 13)
						$("#skillTrafficReport_type").click();
				});

				$("#skillTrafficReport_endTime").keyup(function(event) {
					if (event.keyCode === 13)
						$("#skillTrafficReport_type").click();
				});
